...
func (suite *IntegrationTestSuite) TestHTTPTrustedProxyResolvesRealClient() {
	...
	// Walk-right-to-left should pick 203.0.113.45 (the first non-trusted entry from the right).
	_, err := resty.New().R().
		SetHeader("X-Forwarded-For", "198.51.100.7, 203.0.113.45, 127.0.0.1").
		Get(uri)
	suite.Require().NoError(err)
	...
	suite.Equal("203.0.113.45", matched.SourceIp, "real client must be picked from XFF when peer is trusted")
